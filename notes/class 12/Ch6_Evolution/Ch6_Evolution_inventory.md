...
> |---|---|
> | §1 preamble — `neetenv` rebuilt + verified | done — venv was absent; rebuilt with `uv`; CPython 3.13.11 · reportlab 5.0.1 · pymupdf 1.28.2 · Pillow 12.3.0 (known-good set) |
...
> 2. **Check 4 — portrait/photo row.** False positive: the heuristic keys off the substring "photo" and matched **F161** (*"…similar to the light reaction in **photo**synthesis…"*), a concept row. The chapter has no scientist profile and no photograph; nothing is embedded at all.
>
> **Pass 2 decisions (no inventory row changed):** heading "Moths before and after industrialisation" (H3 under 6.3) is a regrouping banner, NCERT number 6.3 kept visible; key-term icon used only for terms in the summary or an exercise (chemical evolution, fitness, natural selection, homology/branching descent, adaptive radiation, genetic drift, habitat fragmentation, species); two MEMORY AID boxes (selection modes; hominid brain sizes) are marked "not in NCERT"; the species definition in the appendix is labelled as an addition (Rule 5). No `process_flow()` needed the §4.2 fallback.
>
> ## GATE 1 STATUS: **GREEN — CLOSED.**
>
