...
2. **Unnumbered sub-headings spotted in the first half — pointers for session 1a-H.** `Transforming Principle`, `Biochemical Characterisation of Transforming Principle` (both inside §5.2), and the boxed `Central dogma` label on source p. 4. 1a-H must walk the skeleton independently and not rely on this list being complete.
