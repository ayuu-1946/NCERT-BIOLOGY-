# Chapter 13 — Plant Growth and Development Tracker

## Current status

**✅ Gate 3 CLOSED (this session) — chapter is FULLY COMPLETE.** Pass 3(a) (whole-chapter visual render) and Pass 3(b) (bidirectional content cross-check) both ran this session and both returned **zero confirmed defects**. See `## Pass 3(a)` and `## Pass 3(b) / Gate 3 — CLOSED` below and the folded findings in `Ch13_PlantGrowthAndDevelopment_inventory.md`. All five §0.5 deliverables (`.pdf`, `.py`, `_inventory.md`, `assets/`, this tracker) are present in the chapter folder.

**Figure extraction stage:** Complete. Eleven numbered NCERT figures were identified and extracted as eleven standalone assets. All assets are present under `assets/`, are true grayscale PNGs (`mode=L`), and were individually checked through the final contact sheet and targeted full-resolution reviews.

**Three-part crop audit:** Complete. Check A (text-layer word grazing) is clean for all eleven assets. Check B (drawing-extent overflow) is clean for the vector-bearing assets; Figures 13.4 and 13.10 are raster artwork for which the drawing check is not applicable. Check C (6-point border-band ink) is clean for all eleven assets. Figure 13.1 required a documented bottom-edge exception because the lowest root artwork extends into the source caption band; the final crop includes the root tips instead of clipping them.

**Pass 1 / Gate 1:** Complete. The frozen inventory (`Ch13_PlantGrowthAndDevelopment_inventory.md`) holds 84 Facts (F001–F084), 20 headings, 16 openers, the Summary/Exercise-gap classifications, and the 11-figure label matrix.

**Pass 2 / Gate 2:** Complete. `Ch13_PlantGrowthAndDevelopment.py` was written linearly from the frozen inventory, importing the repo-level `neet_template.py`; it renders a 9-page A4 PDF (`Ch13_PlantGrowthAndDevelopment.pdf`) with all eleven figures embedded inline. `check_pdf.py` exits **0 (green gate)**: checks 1–3 and 5–10 all PASS (84/84 Facts ticked, 49/49 figure labels found in running text, all 11 images monochrome, geometry/bands/legibility/orphan-headings/badge-collisions clean). The only WARN is check 4's known portrait-heuristic false positive — the substring "photo" inside the "phototropism"/"photoperiodic" inventory rows — and no person photograph is embedded, matching the accepted Ch12 baseline.

**Pass 3(a) / visual render check:** Complete (this session). Environment rebuilt from scratch (`/vercel/share/neetenv` was absent at session start — the expected §0.2 state; rebuilt and version-verified against the Ch12 baseline: Python 3.13.11, reportlab 5.0.1, pymupdf 1.28.2, Pillow 12.3.0). The PDF was rebuilt from the committed script and `check_pdf.py` re-run: **VERDICT WARN (0 fail, 1 warn)** — the single WARN is check 4's known portrait-heuristic false positive (the substring "photo" inside "phototropism"/"photoperiodic" rows; no person photograph is embedded; all 11 images confirmed monochrome). All **9/9 pages** were then rendered with `pymupdf` (150 dpi colour + a 1-bit B&W threshold render) and inspected individually: zero layout, style-drift, orphaned-heading, overflow, squashed-figure, or print-safety defects. Cross-page style consistency was confirmed for every element type (H1/H2/H3 banners, section-number badges, figure boxes, process-flow triangle badges, the one data table, the one NOTE box). Full findings: `scratch/ch13_gate3/PASS3A_FINDINGS.md`.

**Pass 3(b) / content cross-check:** Complete (this session). A full bidirectional read (source re-extracted with pdfplumber to `/tmp/ch13_pass3/source_text.txt`, 15 pages; no grep or coverage percentage used to clear any row) checked, per section: Direction 1 (inventory → script — MISSING/DRIFTED/FABRICATED) and Direction 2 (source → inventory — UNINVENTORIED). Result: **84/84 Facts COVERED, 0 MISSING, 0 FABRICATED, 0 material DRIFT**; every direction-2 candidate was investigated and either dismissed as a trivial/pedagogical restatement (FP-1…FP-7, all logged with reasoning) or is a transparent, already-inventoried compression (CD-1, kinetin's IUPAC name) or a genuine source-level gap unrelated to the chapter (SRC-1, Exercises 7/9 reference photoperiodism content the rationalised NCERT edition removed from the body). The `intercalary meristem` Summary-only term (NOTE-1) is confirmed already surfaced in the Quick Recap. All 11 figures confirmed at their correct topic with captions matching NCERT wording; `check_pdf.py` check 6 = 49/49 labels in running text. Full findings: `scratch/ch13_gate3/PASS3B_FINDINGS.md`.

**Gate 3 verdict:** Both directions of Pass 3(b) are clean and Pass 3(a) found zero defects across all 10 pages — **0 confirmed defects overall**. No `.py`, inventory Fact, caption, or asset edit was required to close the gate. Per §6, these findings (Pass 3(a) page-by-page table, Pass 3(b) per-section reading claims, false-positive register, and compression/source-gap notes) are folded into `Ch13_PlantGrowthAndDevelopment_inventory.md` as `## Pass 2 / Gate 2`, `## Pass 3(a)` and `## Pass 3(b) / Gate 3 — CLOSED` sections, matching the convention used by every other closed chapter.

**Scope note:** The figure-extraction stage above covered assets only; Pass 2 added the script, the rendered PDF, and the ticked inventory; this session's Pass 3 added the visual and content verification that closes Gate 3.

## Re-pin log

| Asset | Final change | Reason |
|---|---|---|
| `fig_13_1.png` | Final rectangle `(55, 95, 515, 389)` | First crop clipped the lowest root tips. Source geometry confirmed the artwork overlaps the caption baseline; the crop was extended to retain all roots, with the exception recorded in the inventory. |
| `fig_13_2.png` | Final `y1=377` | The lower “Root apical meristem” label and leader line were clipped in the first crop. |
| `fig_13_4.png` | Final `y0=300` | The first crop included the preceding “Growth Rates” prose. The final crop begins at the first figure artwork and retains all three parts and the legend. |
| `fig_13_5.png` | Final `x0=45` | The first crop clipped the vertical “Height of the plant” axis label. |
| `fig_13_6.png` | Final `y1=660` | The initial crop grazed the “Time” axis label; the final crop retains it completely while remaining above the caption. |
| `fig_13_8.png` | Final `y0=78` | The first crop trimmed the top “Cell Division” and “Death” labels and the outer frame. |
| `fig_13_9.png` | Final `y0=425` | The first crop grazed the preceding paragraph; the final crop begins below the prose. |
| `fig_13_10.png` | Final `(52, 525, 295, 660)` | The first crop grazed the preceding heading/prose and produced a right-edge ink warning; the final crop retains all four panels and clears the warning. |
| `fig_13_11.png` | Final `y1=290` | The first crop left the lowest root tips too close to the edge; the final crop gives them a small safety margin while remaining above the caption. |

## Reproduction commands

From the repository root, using the available local environment:

```bash
python3 'notes/class 11/Ch13_PlantGrowthAndDevelopment/extract_figures.py'
python3 'notes/class 11/Ch13_PlantGrowthAndDevelopment/audit_figures.py'
```

The mandatory source grids were rendered at 440 dpi with 5-point coordinate spacing by `scratch/render_ch13_grids.py` and are stored in `scratch/ch13_figs/grid_4x/`.

## Deliverables

The chapter folder contains `extract_figures.py`, `audit_figures.py`, `Ch13_PlantGrowthAndDevelopment_inventory.md`, this tracker, and eleven monochrome PNG assets under `assets/`. Supporting geometry reports, audit logs, grid overlays, the contact sheet, and visual-review findings remain under `scratch/`.

## References

[1]: `../../../../SUPREME COMMAND PROMPT.md` "Repository SUPREME COMMAND PROMPT"
[2]: `../../../../skills/ncert-figure-extraction/SKILL.md` "NCERT figure-extraction skill"
[3]: `../../../../Chapter/class 11/Chapter 13 - Plant Growth and Development.pdf` "NCERT Class 11 Chapter 13 source PDF"
