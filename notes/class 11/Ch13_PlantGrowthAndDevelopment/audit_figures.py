...
spec = importlib.util.spec_from_file_location('ef', 'notes/class 11/Ch13_PlantGrowthAndDevelopment/extract_figures.py')
...
for fid, pno, (x0, y0, x1, y1) in ef.FIGS:
    ...
    for w in page.get_text('words'):
        ...
        if inter.get_area() / max(1e-6, wr.get_area()) <= 0.9:
            cut.append(w[4])
    n = sum(1 for w in page.get_text('words') if not (pymupdf.Rect(*w[:4]) & rect).is_empty)
...
print('--- D) emitted asset checks ---')
for fid, _, _ in ef.FIGS:
    path = os.path.join(ef.OUT_DIR, f'fig_{fid}.png')
    im = Image.open(path)
    print(f'fig_{fid}: exists={os.path.exists(path)} mode={im.mode} size={im.size}')
