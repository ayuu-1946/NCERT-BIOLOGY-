...
This is Pass 2 evidence only (SUPREME COMMAND §6 Pass 3(b) hard bar): a token
...
INV = HERE / "Ch16_ExcretoryProductsAndTheirElimination_inventory.md"
PDF = HERE / "Ch16_ExcretoryProductsAndTheirElimination.pdf"
...
def pdf_text() -> str:
    ...
    doc.close()
    # de-hyphenate line breaks, then flatten
    txt = txt.replace("-\n", "").replace("\n", " ")
    return norm(txt)


def facts_rows(inv: str) -> list[tuple[int, str, str, str, str]]:
    """Return (line_no, id, section, type, wording) for each ## Facts row."""
    out = []
    in_facts = False
...
def main() -> int:
    ...
    for _, fid, sec, typ, wording in rows:
        # score the quoted source wording; strip our own parenthetical asides
        body = re.sub(r"\((?:source|the source|note)[^)]*\)", " ", wording, flags=re.I)
        t = toks(body)
        if not t:
    ...
    if flagged:
        ...
        for fid, sc, missing, sec, typ in sorted(flagged, key=lambda x: x[1]):
            print(f"  {fid}  {sc:5.0%}  §{sec} [{typ}]  missing: {', '.join(missing[:14])}")
        print()
