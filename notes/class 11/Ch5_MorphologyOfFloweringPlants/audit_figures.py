...
for fid,pno,(x0,y0,x1,y1) in ef.FIGS:
 ...
 if not xs: print(f'fig_{fid}: no drawings (raster/text image)'); continue
...
for fid,pno,(x0,y0,x1,y1) in ef.FIGS:
 ...
 for side,b in {'L':(x0-BAND,y0,x0,y1),'R':(x1,y0,x1+BAND,y1),'T':(x0,y0-BAND,x1,y0),'B':(x0,y1,x1,y1+BAND)}.items():
  ...
  pix=page.get_pixmap(clip=r,dpi=DPI); a=np.array(Image.frombytes('RGB',(pix.width,pix.height),pix.samples).convert('L'))
  keep=0; sample=None
