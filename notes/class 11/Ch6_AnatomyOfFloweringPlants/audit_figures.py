"""Three-part audit for Chapter 6 figure crops.

Checks A/B/C follow skills/ncert-figure-extraction/SKILL.md. A is informative for
vector labels/text; B catches clipped vector artwork; C catches unexplained ink
immediately outside each rectangle. The final image-mode check enforces true L.
"""
...
for fid, pno, (x0, y0, x1, y1) in ef.FIGS:
    ...
    for w in page.get_text('words'):
        ...
        if inter.get_area() / max(1e-6, wr.get_area()) <= 0.9:
            cut.append(w[4])
    print(f'fig_{fid}: words_in_rect={len(words_in)}' + (f' GRAZING {cut}' if cut else ' ok'))
...
print('--- D) emitted image mode and existence ---')
for fid, _, _ in ef.FIGS:
    path = os.path.join(HERE, 'assets', f'fig_{fid}.png')
    img = Image.open(path)
    print(f'fig_{fid}: exists={os.path.exists(path)} mode={img.mode} channels={len(img.getbands())} size={img.size}')
