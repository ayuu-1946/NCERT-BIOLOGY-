...
- **`check_pdf.py` exits 0 — VERDICT PASS, 0 fail / 0 warn, green under `--strict`** *(true only at the moment of Gate 2 close, before the verdict table was pasted into the inventory; pasting it re-armed check 4 — see Pass 3 defect **D1**. The settled post-documentation state is exit 0 with **1 inspected benign WARN**, non-strict; `--strict` is exit 1 by design).* All ten checks pass: no footer/header band text · smallest rendered text **6.0pt** · all **6** images monochrome · no person photo · **0** banned Unicode glyphs · figure-label coverage **42/42** (0 partial, 0 missing) · inventory **139/139** Facts rows ticked · **9/9** pages A4 portrait · **53** headings unorphaned · **82** badge plates collision-free.
...
- **One confirmed defect, D1 (audit-trail):** Gate 2's inherited claim of "0 warn, green under `--strict`" was **false when re-tested** — `--strict` returned WARN/exit 1 because the verdict table pasted into the inventory contains the words *portrait*/*photograph*, which check 4 scans for in any table row. The act of documenting Gate 2 invalidated its own claim. Rewording two verdict-table cells (**metadata only — no Facts row, caption or asset touched**) removed those two triggers, but the surviving D1/FP3/C2 rows that *document* the trap must themselves name the trigger words, so check 4 still WARNs on them under `--strict`. **Settled, machine-re-verified state (2026-08-31, closure session): `check_pdf.py` exits 0 with 0 FAIL and 1 inspected benign WARN (non-strict); `--strict` remains exit 1 by design because the inventory documents the trap.** This is the same standard under which Ch1, Ch11, Ch12 and Ch13 closed (each "exit 0, 1 inspected benign warn"); it is **not** strict-clean, and the earlier "genuinely 0/0 under `--strict`" wording is retracted as an over-claim. The WARN is a verified true-negative: all 6 embedded assets are the labelled diagrams `fig_14_1`…`fig_14_5`, and the chapter body has 0 scientist-profile occurrences (FP3).
