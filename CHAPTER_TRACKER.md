...
| 8. Microbes in Human Welfare | ▶️ **PASSES 1 + 2 COMPLETE — GATE 1 GREEN, GATE 2 GREEN (2026-08-23), Gate 3 OPEN** — `Ch8_MicrobesInHumanWelfare.py` (51.6 KB) + **12-page A4 PDF** written linearly from the frozen inventory · `check_pdf.py` **exit 0, 0 fail / 1 inspected WARN** · **198/198 Facts rows ticked** · **17/17 in-figure labels in running text** · 9 embedded images all mono · smallest text 6.0pt · 88 badge plates clear · 37 banner headings all followed by content · no banned glyphs · the lone WARN is **check 4's portrait heuristic** matching the word "photograph" in `F050`/`F203`–`F206` and the four `raster photograph` manifest rows — **inspected and accepted**: the check exists to stop a *scientist portrait* being embedded, and figures 8.4/8.6/8.7 contain **no people at all** while 8.5 is an **industrial plant photograph with two incidental workers**, kept embedded **by explicit user decision** because it is NCERT Figure 8.5 itself and dropping it would break the 9-asset denominator. **Do not "fix" this WARN by removing `fig_8_5`.** Inventory **FROZEN at 207 rows** (`F001..F207`, contiguous, 0 duplicates) = **198 Facts + 9 figure-label matrix rows**; **15 heading rows** (9 numbered + 6 unnumbered) · **14 opener rows** · **8 caption rows** · `Type` vocabulary 10 values, all lowercase · `_extract_labels` parses **17 labels across 5 parsed figure rows** — 9 matrix rows, the four `No in-figure labels` rows (`F203`-`F206`) contributing 0 **by design** — no doubling, no phantom `Fig #` row · 18 summary sentences (16 BODY-PRESENT + 2 SUMMARY-UNIQUE, both folded into `F195`/`F196`) · 4 exercise-gap terms each with a planned home · all 5 sessions ran with machine-derived counts (`1-F`/`1-S`/`1-H`/`1-O`/`1-Z`). **All 198 Facts rows now TICKED (the 9 figure-label matrix rows are not Facts rows and are not counted by check 7); the script and PDF both exist, so Gate 2 was judged and is green. The chapter is still NOT in the Done tally — Gate 3 has not run.** Re-validation fixed one **count restatement** (the inventory twice described the parse as "9 figure rows" where the parser returns 5); no frozen row was touched and no count changed. Figure sweep: **9/9 verified mono assets** (`mode=L`, 300 dpi) covering **8 numbered figures**, 8.2 split into `8_2a` (panels a+b) + `8_2c`; no unnumbered plates, so the denominator is 9 · rects hand-pinned in `extract_figures.py`, each with a comment recording what pinned it · four-part crop gate passed (**A word-grazing · B drawings-extent · B2 raster-extent · C border ink**) — **B2 was added this chapter** because check B reports `no drawings` for the four photographic plates 8.4–8.7 and would have left them mechanically unchecked · check A is **vacuous for 5 of 9 assets** (0 words in rect; every callout is artwork, not text) · **2 rects re-pinned after the gate caught real clipping**: `fig_8_1` (`82,78,…`���`56,76,…`, flagellum sliced off panel c) and `fig_8_3` (`148,…`→`93,…`, petri dish a clipped) · 4 gate hits explained and accepted in writing (page-header motif, neighbouring 8.2c leader artwork, caption tint panels) · all 9 PNGs eyeballed on a fresh contact sheet · figure manifest + caption-verbatim check + figure-label matrix written; 9/9 re-confirmed `mode=L` on disk at re-validation. **Next: Pass 3 — bidirectional content cross-check (read all 12 source pages start to finish, no grep-only clearing) + visual render check of all 12 PDF pages and 9 assets, then Gate 3.** |
