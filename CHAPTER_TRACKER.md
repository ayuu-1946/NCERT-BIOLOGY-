...
| 8. Microbes in Human Welfare | ▶️ **PASS 1 COMPLETE — GATE 1 GREEN (machine re-verified from disk in a later session), Gate 2 OPEN** — inventory **FROZEN at 207 rows** (`F001..F207`, contiguous, 0 duplicates) = **198 Facts + 9 figure-label matrix rows**; **15 heading rows** (9 numbered + 6 unnumbered) · **14 opener rows** · **8 caption rows** · `Type` vocabulary 10 values, all lowercase · `_extract_labels` parses **17 labels across 5 parsed figure rows** — 9 matrix rows, the four `No in-figure labels` rows (`F203`-`F206`) contributing 0 **by design** — no doubling, no phantom `Fig #` row · 18 summary sentences (16 BODY-PRESENT + 2 SUMMARY-UNIQUE, both folded into `F195`/`F196`) · 4 exercise-gap terms each with a planned home · all 5 sessions ran with machine-derived counts (`1-F`/`1-S`/`1-H`/`1-O`/`1-Z`). **All 207 rows correctly UNTICKED — Gate 2 cannot be judged: there is still no `Ch8_MicrobesInHumanWelfare.py` and no PDF, so this chapter is NOT in the Done tally.** Re-validation fixed one **count restatement** (the inventory twice described the parse as "9 figure rows" where the parser returns 5); no frozen row was touched and no count changed. Figure sweep: **9/9 verified mono assets** (`mode=L`, 300 dpi) covering **8 numbered figures**, 8.2 split into `8_2a` (panels a+b) + `8_2c`; no unnumbered plates, so the denominator is 9 · rects hand-pinned in `extract_figures.py`, each with a comment recording what pinned it · four-part crop gate passed (**A word-grazing · B drawings-extent · B2 raster-extent · C border ink**) — **B2 was added this chapter** because check B reports `no drawings` for the four photographic plates 8.4–8.7 and would have left them mechanically unchecked · check A is **vacuous for 5 of 9 assets** (0 words in rect; every callout is artwork, not text) · **2 rects re-pinned after the gate caught real clipping**: `fig_8_1` (`82,78,…`���`56,76,…`, flagellum sliced off panel c) and `fig_8_3` (`148,…`→`93,…`, petri dish a clipped) · 4 gate hits explained and accepted in writing (page-header motif, neighbouring 8.2c leader artwork, caption tint panels) · all 9 PNGs eyeballed on a fresh contact sheet · figure manifest + caption-verbatim check + figure-label matrix written; 9/9 re-confirmed `mode=L` on disk at re-validation. **Next: Pass 2 — write the script from the frozen inventory, ticking rows as written.** |
