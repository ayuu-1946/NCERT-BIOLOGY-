...
**Ch5 tracker-audit note (consistency-only session — no sweep run, no state advanced).** Every Ch5 claim in this roll-up, in `CHAPTER_STATUS.md` and in `Ch5_TRACKER.md` was re-derived from disk: 231 rows `F001..F231`, 0 gaps, 0 dups, 0 ticked; `Type` census sums to 231; 0 heading / 0 opener / 0 figure-label rows; inventory H1 still `# Working Inventory (NOT FROZEN)`; no `.py`, no `.pdf`, no `assets/`; source PDF is 31 pp; the `5.6 GENETIC CODE` banner really is on **p17** (mid-page seam confirmed); all 10 section banners and every §4 sub-heading page match; captions run `Figure 5.1`–`5.16` with `5.4a`/`5.4b` present, confirming **16 numbers / 17 assets**; Done tally re-counted at **11 ✅ rows**. **Three inconsistencies found and fixed** — the Ch5 tracker's heading target was headed "16" above a 17-row table; the inventory reported "1 of 8" sessions where the tracker ledger says 9 (8 sweeps + step-10 re-parse — both now state their basis); and `scratch/ch5mbi/full_text.txt`, committed last session, was recorded in no tracker and is now listed as a **non-deliverable working aid**, not a fact source. **Caveat:** `/vercel/share/neetenv` is absent again (expected per §0.2), so `_extract_labels` was re-checked by replicating its parser under system `python3` — 0 label rows, matching the claim — and must be re-run under the rebuilt venv next session.
