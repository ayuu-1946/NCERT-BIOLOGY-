...
| Workflow stage | Evidence |
|----------------|----------|
| **Environment resume** | Fresh sandbox rebuilt at `/vercel/share/venv`: Python 3.13.11 · `reportlab 5.0.0` · `pdfplumber 0.11.10` · `pymupdf 1.28.2` · `Pillow 12.3.0`. Use `/vercel/share/venv/bin/python`, not bare `python3`. |
| **§0.3 / §4.4 re-verified** | Re-ran the current extractor over all 11 assets. Every reopened PNG is single-channel `mode=L`; dimensions and extrema were checked programmatically, and every asset was freshly inspected for identity, intact labels/leader lines, legibility, contamination, and preserved monochrome meaning. |
| **Fig 3.1 reopened-PNG proof** | The committed `fig_3_1b.png` was stale (`1776×871`) and did not match the current `(64,274,490,491)` extractor crop. Fresh output is `1776×905`. Its three PDF-point scrubs map at 300 dpi to pixel boxes `(1166,812,1304,904)`, `(1191,0,1358,41)`, and `(1037,883,1125,904)`; after save + reopen, each has `min=max=255`, unique values `[255]`. Pixels immediately outside retain source greys/ink, proving the scrubs act at the intended stage and do not erase the nearby `Stipe` / `(b-iii)` labels. Fig 3.1(a), (b), and (c) were inspected independently. |
| **Additional figure fix** | Fresh inspection found Figure 3.5's crop included its printed source caption, violating the no-neighbouring-text requirement. Its bottom edge was tightened from y=488 to y=469, retaining both `(a)` / `(b)` part labels while excluding caption ink that begins below y=470; regenerated asset is `1176×826`, `mode=L`. |
| **Figure-extraction status** | All 11 assets now regenerate from the current extractor and pass the §4.4 asset-level checks. Faint NCERT watermarking was retained: it is source artwork, carries no factual meaning, and does not obscure labels. This completes figure extraction/verification only; **Gate 1 remains closed** until the numbered inventory, figure-label matrix, summary classification, and exercise-gap plan are created and frozen. |
| **Remaining Pass 1 work** | Create and independently audit `Ch3_PlantKingdom_inventory.md`; enter every fact and every in-figure label, classify every summary sentence, and give each exercise-gap term a planned home. Table 3.1 must be read from the source render rather than trusting extracted column order. |
| **Scratch artifacts on disk** | `scratch/ch3/` contains prior working renders; these are non-authoritative and must be removed before final delivery. |
