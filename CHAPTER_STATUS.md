...
| 3 | Plant Kingdom | 11 | ⬜ not started (pre-Gate 1: figure-asset extraction in progress) | ⬜ not started | ⬜ not started | ⬜ none yet (assets/ partial) | **▶️ IN PROGRESS — Pass 1, pre-Gate 1** |
...
## Chapter 3 — Plant Kingdom — ▶️ IN PROGRESS (Pass 1, pre-Gate 1)

**Not yet at Gate 1.** Prior sessions produced a first-read fact inventory in prose (not yet frozen to the numbered `_inventory.md` table) and a figure-extraction script; this session's work was entirely inside §4.4 Step 3 (figure-asset crop verification), which blocks Gate 1 because the figure manifest cannot be marked `Verified: yes` until every crop is confirmed defect-free by eye.

| Workflow stage | Evidence |
|----------------|----------|
| **Environment resume** | Sandbox was fresh (deps not persisted). `pip`/`python3` resolved to a stale Python 3.9; rebuilt a clean venv at `/vercel/share/venv` (`reportlab 5.0.0` · `pdfplumber` · `pymupdf 1.28.2` · `Pillow 12.3.0`) matching the versions used by closed chapters. **Use `/vercel/share/venv/bin/python` in all future sessions**, not bare `python3`. |
| **Branch hygiene issue** | This session ran `git checkout -b ch3-plant-kingdom` before realising the connected head branch is `plant-kingdom-curriculum`, which was already the checked-out branch. All real work happened on `plant-kingdom-curriculum`. The stray local branch `ch3-plant-kingdom` (pointing at the pre-session commit `8a55467`) is unused dead weight — safe to delete (`git branch -D ch3-plant-kingdom`), never pushed. |
| **§0.3 re-verified** | All 11 committed assets under `notes/class 11/Ch3_PlantKingdom/assets/` confirmed `mode=L`, 0 colored pixels (monochrome conversion holds). |
| **Fig 3.1 (Volvox/Laminaria/Porphyra) — geometry fully re-derived from measured ink, not guesswork** | Two prior sessions' crops for `fig_3_1a/b/c` all had real defects (labels clipped, neighbouring-figure intrusion, or a figure sliced in half) because the source is a single unlabelled raster (`page 3 image bbox x68.8–494.7, y102.0–672.3`) with sub-parts laid out as **interleaved columns, not clean rows** — row (a)'s "(a-ii)" label sits at y270–287 *inside* row (b)'s naive y-range, and (c-ii) *Polysiphonia* is tall enough to reach up into row (b)'s band on the right. This session ran a thresholded ink-profile scan (PIL, threshold<150 to reject the faint NCERT watermark) plus multiple coordinate-gridded renders (150–600 dpi) to measure every sub-part's true bbox, and rewrote `extract_figures.py`'s Fig 3.1 rects + a detailed geometry comment block accordingly: `fig_3_1a → (88, 106, 392, 293)`, `fig_3_1b → (64, 290, 490, 491)` + one scrub rect, `fig_3_1c → (98, 484, 452, 675)`. Re-extraction confirmed (by eye) that `fig_3_1a` and `fig_3_1c` are now clean — all labels present, no intrusion, no clipping. |
| **⚠️ Known open defect — do not trust as-is** | The `fig_3_1b` scrub rect currently committed, `(383.0, 469.0, 468.0, 491.0)`, was written to remove an assumed "(c-ii) intrusion" but a follow-up 600 dpi zoom (`/tmp/v3/scrubzone.png`, not persisted) showed that zone contains **only `fig_3_1b`'s own ink** — the "Stipe" label and the "(b-iii)" part-label — so this scrub rect is currently **erasing real content that belongs in the asset**, not fixing a defect. The session was in the middle of re-locating where the actual (c-ii) top-right intrusion sits (had rendered `/tmp/v3/fullband.png`, a gridded y430–560 boundary strip, but had not yet read/interpreted it) when it ended. **`fig_3_1b` must be re-verified by eye before Gate 1** — do not assume the current committed crop is correct just because it re-extracts without error. |
| **Not yet touched this session** | Figs 3.2 (Marchantia + mosses), 3.3 (Selaginella/fern), 3.4 (Cycas/Pinus/Ginkgo), 3.5 — all previously extracted and reportedly clean per prior-session notes, but **not independently re-verified this cycle**; treat as unconfirmed until eyeballed. Also untouched: freezing the numbered `Ch3_inventory.md` Facts table, the figure-label matrix, exercise-gap terms (Q2, Q4, Q8(iv) flagged in a prior session as needing real appendix work since the body text doesn't cover them), and Table 3.1's column-shuffled-in-extraction values (already hand-verified from the render in a prior session: 7 columns × 3 data rows, title says "Divisions" while columns say "Classes" — an NCERT internal inconsistency to preserve verbatim). |
| **Scratch artifacts on disk** | `scratch/ch3/{renders,verify,v2}/` and `scratch/ch3/{grid_p3.png,source_text.txt}` hold prior-session working renders — useful reference, not authoritative (superseded by this session's freshly-measured Fig 3.1 geometry). Not cleaned up; still throwaway per §0.5, delete before final delivery. |
