...
SPEC='notes/class 11/Ch14_BreathingAndExchangeOfGases/extract_figures.py'
...
lines=[]
lines.append('--- A) text-layer word grazing ---')
for fid,pno,(x0,y0,x1,y1) in ef.FIGS:
    page=doc[pno-1]; rect=pymupdf.Rect(x0,y0,x1,y1); cut=[]; inside=[]
...
os.makedirs('scratch/ch14_figs',exist_ok=True)
open('scratch/ch14_figs/audit_results.txt','w').write('\n'.join(lines)+'\n')
