...
SCRIPT='notes/class 11/Ch12_RespirationInPlants/extract_figures.py'
...
try:
 ...
 for fid,pno,(x0,y0,x1,y1) in ef.FIGS:
  ...
  status='GRAZING '+repr(cut) if cut else 'ok'
  print(f'fig_{fid}: words_in_rect={len(inside)} {status}')
  results.append([fid,len(inside),status,'','',''])
 print('--- B) drawings-extent overflow ---')
 for row,(fid,pno,(x0,y0,x1,y1)) in zip(results,ef.FIGS):
  ...
  if not xs: s='no drawings (raster or artwork-as-image)'
  else:
   ov=[max(0,x0-min(xs)),max(0,y0-min(ys)),max(0,max(xs)-x1),max(0,max(ys)-y1)]
   s='OVERFLOW L%.1f T%.1f R%.1f B%.1f'%tuple(ov) if max(ov)>3 else 'ok'
  row[3]=s; print(f'fig_{fid}: {s}')
 ...
 for row,(fid,pno,(x0,y0,x1,y1)) in zip(results,ef.FIGS):
  ...
  s='EDGE-INK '+repr(hits) if hits else 'clean'; row[4]=s; print(f'fig_{fid}: {s}')
 print('--- D) grayscale and files ---')
 for row,(fid,_,_) in zip(results,ef.FIGS):
  path=os.path.join(ef.OUT_DIR,f'fig_{fid}.png'); im=Image.open(path)
  s=f'mode={im.mode} size={im.size}'
  row[5]=s; print(f'fig_{fid}: {s}')
finally: doc.close()
